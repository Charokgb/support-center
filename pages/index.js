...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-custom-bg bg-cover bg-center">
      {/* 👆 bg-custom-bg comes from your tailwind.config.js extension */}
      ...
      <div className="min-h-screen bg-black/50">
        <div className="max-w-5xl mx-auto p-6 text-white">
          ...
          <section className="bg-white/90 text-black p-6 rounded shadow mb-8">
                  ...
                  {results.map(r => (
                    <li key={r.id} className="mb-2">
                      <Link href={`/article/${r.id}`}>
                        <a className="text-blue-600">{r.title}</a>
                      </Link>
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {categories.map(cat => (
              <Link key={cat.slug} href={`/category/${cat.slug}`}>
                <a className="block bg-white/90 rounded p-4 shadow hover:shadow-md text-black">
